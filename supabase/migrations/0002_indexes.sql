...
-- Stripe webhook lookups (partial — only rows that have a customer id)
...
-- Role lookups: every RLS policy check fires this path
CREATE INDEX idx_memberships_user_role ON public.memberships(user_id, role);

-- Primary membership fast-path (used by get_user_context)
CREATE INDEX idx_memberships_primary ON public.memberships(user_id)
  WHERE is_primary = true;

-- Uniqueness: one tenant-wide membership per user (facility_id IS NULL)
CREATE UNIQUE INDEX memberships_unique_tenant_wide
  ON public.memberships(user_id, tenant_id)
  WHERE facility_id IS NULL;

-- Uniqueness: one facility-specific membership per user
CREATE UNIQUE INDEX memberships_unique_facility_specific
  ON public.memberships(user_id, tenant_id, facility_id)
  WHERE facility_id IS NOT NULL;

-- Uniqueness: at most one primary membership per user
CREATE UNIQUE INDEX memberships_unique_primary
...
CREATE INDEX idx_products_facility_stock ON public.products(facility_id, stock_quantity)
  WHERE track_inventory = true;
CREATE INDEX idx_products_available      ON public.products(facility_id, name)
...
CREATE INDEX idx_register_sessions_active   ON public.register_sessions(facility_id, opened_at)
...
CREATE INDEX idx_order_items_facility ON public.order_items(facility_id);
...
CREATE INDEX idx_bookings_active           ON public.bookings(facility_id, starts_at)
  WHERE status IN ('confirmed', 'pending');
CREATE INDEX idx_bookings_details          ON public.bookings USING gin(details);
CREATE INDEX idx_bookings_customer_trigram ON public.bookings
  USING gin(customer_name extensions.gin_trgm_ops);

-- Football field overlap prevention (per-field, scoped by details->>'field_number')
...
-- General facility overlap prevention for all non-football bookings
-- (birthdays, events — treated as single-space bookings)
ALTER TABLE public.bookings ADD CONSTRAINT no_overlapping_bookings
  EXCLUDE USING gist (
    facility_id WITH =,
    tstzrange(starts_at, ends_at) WITH &&
  ) WHERE (type != 'football' AND status NOT IN ('canceled', 'no_show'));
