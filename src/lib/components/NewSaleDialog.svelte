...
<DialogRoot bind:open={open}>
  ...
  >
    ...
    <div class="grid flex-1 min-h-0 grid-cols-1 gap-4 overflow-hidden px-6 pb-6 pt-4 md:grid-cols-[240px_1fr_360px]">
      <aside class="flex min-h-0 flex-col gap-3 rounded-xl border border-outline-soft/60 bg-background/80 p-3">
            ...
            <div class="space-y-1">
              ...
              {#each categories.filter((c) => c.parent_id === cat.id) as sub}
                <button
                  ...
                  onclick={() => (selectedCategory = sub.id)}
                >
      ...
      <section class="min-h-0 overflow-auto rounded-xl border border-outline-soft/60 bg-background/70 p-4">
        <div class="grid content-start gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {#each filteredProducts as p}
            ...
            >
              {#if p.image_url}
                <div class="flex h-28 w-full items-center justify-center bg-muted/30">
                  <img src={p.image_url} alt={p.name} class="h-full w-full object-cover" loading="lazy" />
                </div>
              {:else}
                <div class="h-28 w-full bg-muted/20"></div>
              {/if}
              <div class="flex flex-1 flex-col justify-between gap-1 px-3 py-3 text-left">
      ...
      <aside class="flex min-h-0 flex-col rounded-xl border border-outline-soft/60 bg-background/80 p-4">
        ...
        <div class="flex-1 space-y-2 overflow-auto pr-1">
          {#if cart.length === 0}
            <div class="grid place-items-center gap-2 rounded-lg border border-dashed border-outline-soft/60 bg-surface px-6 py-10 text-center text-sm text-muted-foreground">
              <ShoppingCart class="size-10 opacity-50" />
              <p>{t("orders.emptyCart")}</p>
            </div>
          {:else}
            {#each cart as item, index}
              <div class="flex items-center justify-between gap-3 rounded-lg border border-outline-soft/40 bg-surface px-3 py-2 text-sm">
                <div class="min-w-0">
                  <div class="truncate font-medium text-foreground">{item.name}</div>
        ...
        <div class="mt-4 space-y-3">
          <div class="flex items-center justify-between">
            ...
            <div class="inline-flex items-center gap-2 rounded-lg border border-outline-soft/60 bg-surface px-2 py-1">
              <Button
                type="button"
                size="icon"
                variant="ghost"
              ...
              <span class="w-6 text-center text-sm font-semibold">{couponCount}</span>
              <Button
                type="button"
                size="icon"
                class="size-7 rounded-md"
          ...
          <div class="space-y-1 rounded-lg border border-outline-soft/50 bg-surface px-3 py-3 text-sm">
            ...
            <div class="flex justify-between border-t border-outline-soft/40 pt-2 text-base font-semibold">
        ...
        <DialogFooter class="mt-4 flex justify-end gap-2 border-t border-outline-soft/50 pt-3">
          <Button type="button" variant="outline" class="rounded-lg" onclick={() => (open = false)}>
            {t("orders.close")}
          </Button>
          <Button type="button" class="rounded-lg px-6" onclick={submit} disabled={cart.length === 0}>
