...
let { order } = $props<{ order: OrderRow }>();
...
<div class="flex flex-col gap-4 p-6">
  ...
  <div class="grid gap-2 rounded-xl border border-outline-soft bg-surface/70 p-3 text-xs font-medium text-muted-foreground sm:grid-cols-3">
    ...
    <div class="flex flex-col gap-1">
      ...
      <span class="text-base font-semibold text-foreground">{items.filter((i) => i.is_treat).length}</span>
    </div>
  </div>
    ...
    {#each items as item}
      <div class="flex items-center justify-between gap-4 rounded-xl border border-outline-soft bg-surface px-3 py-2 text-sm">
        ...
        <div class="flex items-center gap-2 text-sm">
          {#if item.is_treat}
            <span class="text-xs text-muted-foreground line-through">{money(item.unit_price)}</span>
            <span class="font-medium text-foreground">€0.00</span>
          {:else}
            <span class="font-medium text-foreground">{money(item.line_total)}</span>
          {/if}
        </div>
      </div>
    {/each}
