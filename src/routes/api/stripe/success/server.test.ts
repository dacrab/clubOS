...
vi.mock("$lib/server/supabase-admin", () => ({
	getSupabaseAdmin: () => ({ from: () => ({ upsert: mockUpsert }) }),
}));
...
describe("GET /api/stripe/success", () => {
	it("redirects on Stripe API error", async () => {
		...
		await expect(
			GET({ url: makeUrl("sess_123"), locals: mockLocals } as never),
		).rejects.toMatchObject({ location: "/billing?error=payment_failed" });
	});
	...
	it("upserts subscription and redirects to admin", async () => {
		...
		await expect(
			GET({ url: makeUrl("sess_123"), locals: mockLocals } as never),
		).rejects.toMatchObject({ location: "/admin?welcome=true" });
	...
	it("returns error redirect when session_id is missing", async () => {
		...
		await expect(
			GET({ url: urlWithoutSession, locals: mockLocals } as never),
	...
	it("redirects to billing with error when subscription lacks tenant_id in metadata", async () => {
		...
		await expect(
			GET({ url: makeUrl("sess_123"), locals: mockLocals } as never),
		).rejects.toMatchObject({ location: "/billing?error=missing_tenant" });
	});
	...
	it("redirects to billing with error when user is not authenticated", async () => {
		...
		await expect(
			GET({ url: makeUrl("sess_123"), locals: unauthLocals } as never),
		).rejects.toMatchObject({ location: "/billing?error=missing_tenant" });
	});
});
