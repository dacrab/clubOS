...
describe("POST /api/stripe/checkout", () => {
	...
	const setupMocks = (
		opts: { customerOk?: boolean; sessionOk?: boolean; customerId?: string } = {},
	): void => {
		...
		mockFetch.mockImplementation((url: string) => {
			if (url.includes("/v1/customers"))
				return Promise.resolve({
					...
					json: () =>
						Promise.resolve(
							customerOk ? { id: customerId } : { error: { message: "Customer failed" } },
						),
				});
	...
	it("creates customer and session for new user", async () => {
		...
		const response = await POST(
			req({ priceId: "price_test", userId: user.id, email: user.email }, user) as never,
		);
	...
	it("reuses existing customer from subscription", async () => {
		...
		const { request } = req(
			{ priceId: "price_test", userId: user.id, email: user.email, tenantId: tenant.id },
			user,
		);
		const locals = createMockLocals({
			user,
			tenants: [tenant],
			subscriptions: [{ tenantId: tenant.id, status: "trialing", stripeCustomerId: existingId }],
		});
		const { POST } = await import("./+server");
	...
	it("creates new customer if existing has temp_ prefix", async () => {
		...
		const locals = createMockLocals({
			user,
			tenants: [tenant],
			subscriptions: [{ tenantId: tenant.id, status: "trialing", stripeCustomerId: "temp_x" }],
		});
		const { POST } = await import("./+server");
		await POST({ request, locals } as never);
	...
	it.each([
		...
		[
			"userId",
			(u: ReturnType<typeof createMockUser>) => ({ priceId: "price_test", email: u.email }),
		],
