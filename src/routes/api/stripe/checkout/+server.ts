...
export const POST: RequestHandler = async ({ request, locals }) => {
	const { priceId, userId, email, tenantId } = (await request.json()) as {
		priceId: string;
		userId: string;
		email: string;
		tenantId?: string;
	...
	try {
		...
		if (tenantId) {
			...
			if (sub?.stripe_customer_id && !sub.stripe_customer_id.startsWith("temp_")) {
				...
				const c = await stripePost("/customers", key, {
					email,
		...
		if (tenantId) sessionParams["subscription_data[metadata][tenant_id]"] = tenantId;

		const session = await stripePost("/checkout/sessions", key, sessionParams);
